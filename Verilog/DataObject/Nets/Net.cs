...
namespace pluginVerilog.Verilog.Nets
{
    public class Net : Variables.IVariableOrNet
    {
        ...
        public Variables.DataTypes.DataTypeEnum DataType;

        public List<Variables.Range> PackedDimensions { get; set; } = new List<Variables.Range>();
        public Variables.Range Range
        {
            get
            {
        ...
        public void AppendLabel(ColorLabel label)
        {
            ...
            foreach (Variables.Range dimension in Dimensions)
            {
                label.AppendText(" ");
        ...
        public static Net Create(NetTypeEnum netType, Variables.DataTypes.DataType dataType)
        {
            Net net = new Net();
            net.NetType = netType;
            net.DataType = dataType.Type;
            if(dataType is Variables.DataTypes.IntegerVectorType)
            {
                var integerVectorType = (Variables.DataTypes.IntegerVectorType) dataType;
                net.PackedDimensions = integerVectorType.PackedDimensions;
                net.Signed = integerVectorType.Signed;
            }else if (dataType is Variables.DataTypes.IntegerAtomType )
            {
                var integerAtomType = (Variables.DataTypes.IntegerAtomType)dataType;
        ...
        public static bool ParseDeclaration(WordScanner word, NameSpace nameSpace)
        {
            ...
            // [range]
            Variables.Range range = null;
            if (word.GetCharAt(0) == '[')
            {
                range = Variables.Range.ParseCreate(word, nameSpace);
            ...
            while (!word.Eof)
            {
                ...
                nets.Add(net);
                net.Signed = signed;
                net.PackedDimensions.Add(range);
                net.Name = word.Text;
                net.NetType = netType;
                ...
                if (word.Active)
                {
                    if (word.Prototype)
                    {
                        ...
                        {
                            nameSpace.Nets.Add(net.Name, net);
                        }
                        word.Color(CodeDrawStyle.ColorType.Net);
                    }
                    else
                    {
                        if (nameSpace.Variables.ContainsKey(net.Name) && nameSpace.Nets[net.Name] is Net)
                        {
                            net = nameSpace.Nets[net.Name] as Net;
                        }
                        word.Color(CodeDrawStyle.ColorType.Net);
                    }
                }
                ...
                else if (word.Text == "[")
                {
                    while (word.Text == "[")
                    {
                        Variables.Range dimension = Variables.Range.ParseCreate(word, nameSpace);
