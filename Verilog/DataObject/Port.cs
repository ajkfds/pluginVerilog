    ...
    public class Port : Item, CommentAnnotated
    {
        ...
        public Range Range
        {
            get
            {
                ...
                {
                    if( (VariableOrNet as Variable) is Variables.IntegerVectorValueVariable)
                    {
...
| [ port_direction ] . port_identifier ( [ expression ] )         
        ...
        {
            DataTypes.DataType prevDataType = null;
            Nets.Net.NetTypeEnum? prevNetType = null;
        ...
        private static bool ParsePortDeclaration(WordScanner word, NameSpace nameSpace, bool firstPort, ref DataTypes.DataType prevDataType, ref Nets.Net.NetTypeEnum? prevNetType, ref DirectionEnum? prevDirection)
        {
            ...
            Nets.Net.NetTypeEnum? netType = Nets.Net.parseNetType(word, nameSpace);

            DataTypes.DataType dataType = null;

            if ( firstPort && direction == null && netType == null && dataType == null)
            {
                if (firstPort)
                {
            ...
            {
                dataType = DataTypes.DataType.ParseCreate(word, nameSpace);
            }
            ...
            if(netType != null)
            {
                port.VariableOrNet = Nets.Net.Create((Nets.Net.NetTypeEnum)netType, dataType);
            }
            else if(dataType != null)
            {
                port.VariableOrNet = Variable.Create(dataType);
            }
        ...
        public static void ParseTfPortItem(WordScanner word, NameSpace nameSpace,bool first,DirectionEnum prevDirection,DataTypes.DataType prevDataType)
        {
            BuildingBlock buildingBlock = nameSpace.BuildingBlock;
            DirectionEnum? direction = null;
            switch (word.Text)
            ...
            if(dataType == null)
            {
                if(first || direction != null)
                {
                    ...
                    vectorType.Type = DataTypes.DataTypeEnum.Logic;
